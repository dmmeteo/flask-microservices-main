...
test(`should display flash messages correctly`, async(t) => {
    ...
    await t
        ...
        .typeText('input[name="email"]', 'incorrect@emai.com')
        .typeText('input[name="password"]', password)
        .click(Selector('input[type="submit"]'))
    ...
    await t
        .expect(Selector('.alert-success').exists).ok()
        .expect(Selector('.alert-danger').withText('User does not exist.').exists).notOk()
